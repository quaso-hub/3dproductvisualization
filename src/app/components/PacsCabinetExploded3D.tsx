...
function buildDoorAssembly(scene: THREE.Scene, xCenter: number, zCenter: number, isRightDoor: boolean) {
  ...
  [-6, 6].forEach(dy => {
    addBox(scene, 1.5, 1.5, 2.5, handleX, DOOR_Y + dy, zCenter + 1, matSSPolished());
  });
  ...
  if (isRightDoor) {
    addBox(scene, 3, 4, 2, handleX, DOOR_Y - HANDLE_LEN / 2 - 4, zCenter + 1, matSSPolished());
  }
  ...
  [DOOR_Y + DOOR_H / 2 - 10, DOOR_Y, DOOR_Y - DOOR_H / 2 + 10].forEach(hy => {
    addBox(scene, 1, 6, DOOR_T * 0.8, hingeX + hingeSign * 0.5, hy, zCenter, matSSPolished(), false);
  });
}
...
function buildScene(scene: THREE.Scene, renderer: THREE.WebGLRenderer) {
  ...
  buildDoorAssembly(scene, leftDoorExX, leftDoorExZ, false);
  ...
  buildDoorAssembly(scene, rightDoorExX, rightDoorExZ, true);
  ...
  SHELF_YS.forEach(sy => {
    addBox(scene, SHELF_W, SHELF_T, SHELF_D, 0, sy, shelfExZ, shelfMat);
