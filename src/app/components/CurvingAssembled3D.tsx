...
function buildAssembledScene(scene: THREE.Scene, L: number) {
  ...
  // Hero geometry: anodized aluminum L-profile with R40 inner fillet.
  // Wrapped in Group so highlight-controller can pick the whole profile
  // when its label ('Aluminium Angle 40×40' or 'Anodized Coating') is hovered.
  const angleGroup = new THREE.Group();
  angleGroup.userData.partId = 'aluminium-angle';
  ...
  angleGroup.add(profileMesh);

  // Subtle edge definition (LineSegments — not a Mesh, won't be raycast-picked
  // but inherits partId via parent walk if added to the group). Keep at scene
  // level so it doesn't bias highlight visuals.
  ...
  // Two distinct seal lines (one along floor edge, one along wall edge).
  // Each gets its own partId so labels resolve unambiguously.
  ...
  const siliHGroup = new THREE.Group();
  siliHGroup.userData.partId = 'silicone-horizontal';
  scene.add(siliHGroup);
  ...
  const siliVGroup = new THREE.Group();
  siliVGroup.userData.partId = 'silicone-vertical';
  scene.add(siliVGroup);
  ...
  siliVGroup.add(siliV);

  // - 3. Pop Rivets --------------------─
  // All rivets (both wings) share one partId — they are conceptually one
  // fastener system. Hovering 'Pop Rivets' highlights every rivet.
  const rivetsGroup = new THREE.Group();
  rivetsGroup.userData.partId = 'pop-rivets';
  scene.add(rivetsGroup);
  ...
  for (let i = 0; i <= nRivets; i++) {
    ...
    rivetsGroup.add(capB);
  }

  // - 4. Annotations (CSS2D) ---------------─
  // Each annotation carries the partId of the Group that owns its anchor mesh,
  // so the highlight controller can wire bidirectional label↔3D selection.
  // 'Aluminium Angle' and 'Anodized Coating' both point at the same partId
  // because anodization is a surface property of the angle, not a separate mesh.
  const zA = L * 0.28;

  placeAnnotations(
    scene,
    [
      { partId: 'aluminium-angle',    anchor: new THREE.Vector3(-W / 2, W * 0.6, zA),           label: 'Aluminium Angle 40×40' },
      { partId: 'aluminium-angle',    anchor: new THREE.Vector3(-W - 0.5, W * 0.4, zA),         label: 'Anodized Coating' },
      { partId: 'silicone-horizontal', anchor: new THREE.Vector3(-W / 2 + T / 2, -0.75, zA),     label: 'White Silicone' },
      { partId: 'pop-rivets',         anchor: new THREE.Vector3(W * 0.35, -(T / 2 + 1), zA),    label: 'Pop Rivets' },
      { partId: 'silicone-vertical',  anchor: new THREE.Vector3(-W - 0.75, T + R + (W - T - R) * 0.5, zA), label: 'Sealant Vertikal' },
    ],
    W + 60,
    [-W * 0.8, W * 1.2],
  );
