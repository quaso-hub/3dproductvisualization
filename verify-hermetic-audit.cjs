...
const OUT = path.join(__dirname, 'visual-inspection-screenshots-hermetic-audit');
...
(async () => {
  ...
  for (const preset of ['ISOMETRIC', 'TAMPAK DEPAN', 'TAMPAK SAMPING', 'DETAIL TRACK', 'DETAIL ATAS', 'OPEN 50%']) {
    if (await clickButtonByText(page, preset)) {
      await new Promise((r) => setTimeout(r, 2000));
      await page.screenshot({ path: path.join(OUT, `assembled-${preset.toLowerCase().replace(/\s+/g, '-').replace('%','pct')}.png`) });
    }
  }
